...
function makePrompt() {
  ...
  return `[[b;#d33682;]${environment.user || "user"}]@[[b;#6c71c4;]${
    environment.hostname || settings.domain || "node-web-console"
  }]${path || "~"}$ `;
}
  ...
  $.jrpc(
    ...
    function (json) {
      ...
      if (!json.error) {
        ...
        terminal.error(
          `&#91;ERROR&#93; RPC: ${message || "Unknown error"}${
            data ? " (" + data + ")" : ""
          }`
        );
      }
    ...
    function (xhr, status, errorData) {
      ...
      if (error) {
        ...
        if (status !== "abort") {
          ...
          terminal.error(
            `&#91;ERROR&#93; AJAX: ${status || "Unknown error"}${
              response ? "\nServer reponse:\n" + response : ""
            }`
          );
        }
